...
export default function QREntryPage() {
  ...
  const processEntry = useCallback(async (user: { name: string; email: string }) => {
    try {
      await addDoc(collection(db, "employeeActivity"), {
      ...
      setStatus('processed');
      setTimeout(() => router.push("/employee-activity"), 1500);
    } catch (error) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (user && user.email) {
        setUserForDisplay({ name: user.displayName || user.email, email: user.email });
        setStatus('processing');
        processEntry({ name: user.displayName || user.email, email: user.email });
      } else {
